...
func recordEvent(recorder events.Recorder, obj client.Object, eventType string, notification bool, err error, reason string) {
	...
	case corev1.EventTypeNormal:
		if notification {
			...
			recorder.Eventf(obj, nil, corev1.EventTypeNormal, reason, eventv1.ActionApplied, "%s", err.Error())
		} else {
			// K8s native event only.
			recorder.Eventf(obj, nil, eventv1.EventTypeTrace, reason, eventv1.ActionApplied, "%s", err.Error())
		}
	case corev1.EventTypeWarning:
		...
		recorder.Eventf(obj, nil, corev1.EventTypeWarning, reason, eventv1.ActionFailed, "%s", err.Error())
	}
}
